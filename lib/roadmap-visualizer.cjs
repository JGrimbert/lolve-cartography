...
/**
 * Calcule les positions (x, y) de chaque step selon le DAG
 * @param {Object[]} steps
 * @returns {{ [id]: {x, y, level} }}
 */
function computeLayout(steps) {
  // 1. Niveau topologique (longest-path layer assignment)
  const level = {};
  const getLevel = (id) => {
    if (level[id] !== undefined) return level[id];
    const s = steps.find(s => s.id === id);
    if (!s || s.dependsOnSteps.length === 0) { level[id] = 0; return 0; }
    level[id] = Math.max(...s.dependsOnSteps.map(getLevel)) + 1;
    return level[id];
  };
  ...
  // 3. Trier dans chaque niveau par id
  for (const l of Object.keys(byLevel)) byLevel[l].sort((a, b) => a.id - b.id);

  // 4. Calculer la largeur totale
  const maxPerLevel = Math.max(...Object.values(byLevel).map(arr => arr.length));
  const totalW = H_PAD * 2 + maxPerLevel * MIN_COL_W;

  // 5. Affecter les positions
  const pos = {};
  for (const [l, arr] of Object.entries(byLevel)) {
    const spacing = (totalW - 2 * H_PAD) / arr.length;
    arr.forEach((s, i) => {
      pos[s.id] = {
        x: H_PAD + (i + 0.5) * spacing,
        y: V_PAD + Number(l) * LEVEL_H,
        level: Number(l),
      };
    });
  }

  return { pos, totalW, totalH: V_PAD * 2 + (Math.max(...Object.keys(byLevel).map(Number)) || 0) * LEVEL_H };
}
