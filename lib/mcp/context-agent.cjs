...
class ContextAgent {
  ...
  // ==========================================================================
  // PROJECT CONTEXT (Symbol Map + Call Graph)
  // ==========================================================================

  /**
   * Génère un contexte projet complet pour un prompt LLM.
   * Utilise l'index de méthodes existant (MethodIndexer) pour le Symbol Map
   * et CallGraph (Acorn AST) pour les relations d'appel inter-classes.
   *
   * @param {Object} options
   * @param {Object|null} options.conceptMap - { NomConcept: "définition", ... }
   * @param {number} options.maxCalleesPerMethod - Limite d'arêtes par méthode (défaut: 5)
   * @returns {string} Contexte formaté en blocs === ... ===
   */
  formatProjectContext({ conceptMap = null, maxCalleesPerMethod = 5 } = {}) {
    const methods = this.methodIndexer.index.methods || {};

    // 1. Symbol Map depuis l'index (classes → [signatures])
    const symbolMap = {};
    for (const [key, method] of Object.entries(methods)) {
      if (method.isPrivate) continue;
      const cls = key.split('.')[0];
      if (!symbolMap[cls]) symbolMap[cls] = [];
      symbolMap[cls].push(method.signature || key.split('.')[1]);
    }

    // 2. Classes connues pour le call graph
    const knownClasses = new Set(Object.keys(symbolMap));

    // 3. Call graph via Acorn
    const srcPath = path.join(this.rootPath, this.config.project.srcPath || 'src');
    const callGraph = buildCallGraph(srcPath, knownClasses);
    ...
    if (conceptMap && Object.keys(conceptMap).length > 0) {
      lines.push('=== CONCEPTS ===');
      for (const [name, def] of Object.entries(conceptMap)) {
        lines.push(`${name} : ${def}`);
      }
      lines.push('');
    }
    ...
    if (Object.keys(symbolMap).length > 0) {
      ...
      for (const [cls, sigs] of Object.entries(symbolMap).sort()) {
        lines.push(cls);
        for (const sig of sigs) lines.push(`  ${sig}`);
      }
      lines.push('');
    }
    ...
    if (Object.keys(callGraph).length > 0) {
      ...
      for (const [caller, callees] of Object.entries(callGraph).sort()) {
        lines.push(caller);
        for (const c of callees.slice(0, maxCalleesPerMethod)) lines.push(`  → ${c}`);
      }
      lines.push('');
    }
