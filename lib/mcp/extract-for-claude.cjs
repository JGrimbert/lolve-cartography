...
const { loadProjectConfig, deepMerge } = require('../utils/config-loader.cjs');
...
class ExtractForClaude {
  ...
  async init() {
    ...
    const moduleConfig = readJSON(this.configPath) || {};

    // Charger et fusionner avec la configuration du projet
    const projectConfig = loadProjectConfig(process.cwd());
    this.config = deepMerge(moduleConfig, projectConfig);
