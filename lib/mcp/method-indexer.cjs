...
function findLeadingCommentsStart(content, nodeStart) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (line.endsWith('*/')) {
      ...
      // Stop ici — ne pas remonter dans le JSDoc de la fonction précédente
      break;
    }
...
class ASTParser {
  ...
  parseFile(filePath) {
    ...
    try {
      ...
      const ast = isTS
        ? TSParser.parse(content, CONFIG.acornOptions)
        : acorn.parse(content, CONFIG.acornOptions);
      return { ast, content };
    } catch (err) {
   ...
   * Note: acorn-typescript may produce inverted start/end for ObjectPattern params in TS files.
   * In that case, fall back to the param name or type.
   */
  paramToString(param, content) {
    if (param.start >= 0 && param.end > param.start) {
      return extractCode(content, param.start, param.end);
    }
    // Fallback for invalid positions (acorn-typescript bug with destructured TS params)
...
class MethodIndexer {
  ...
  indexFile(filePath) {
    ...
    // Build a route prefix for HTTP handler names to avoid collisions across files
    // e.g. src/routes/api/tasks/[id]/merge/+server.ts -> "tasks.[id].merge"
    const HTTP_METHODS = new Set(['GET','POST','PUT','PATCH','DELETE','HEAD','OPTIONS']);
    const isRouteFile = path.basename(filePath).startsWith('+') && relativePath.includes('routes');
    let routePrefix = '';
    if (isRouteFile) {
      const dir = path.dirname(relativePath);
      routePrefix = dir
        .replace(/^src[/\\]routes[/\\]?/, '')  // strip src/routes/
        .replace(/[/\\]/g, '.')
        .replace(/\s/g, '');
    }
    ...
    for (const func of functions) {
      // Use route-qualified key for HTTP handlers to avoid multi-file collisions
      const funcKey = (isRouteFile && HTTP_METHODS.has(func.name) && routePrefix)
        ? `${routePrefix}.${func.name}`
        : func.fullName;
      this.index.methods[funcKey] = {
        file: relativePath,
        class: null,
        name: func.name,
