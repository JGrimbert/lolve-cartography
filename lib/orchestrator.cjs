...
const { ExperienceMemory, RiskInferenceEngine } = require('./experience-memory.cjs');
...
Note: L'optimisation par méthode réduit la consommation de tokens de ~95%,
  ...
  async run(query) {
    ...
    try {
      ...
      // 2. Recherche de méthodes pertinentes (pas fichiers entiers !)
      ...
      // Contexte initial avec les méthodes trouvées
      const methodsContext = {
        methodCount,
        methods: searchSession.getAtLevel(2),
        detectedCategory: this.agents.context.detectQueryCategory(query.toLowerCase())
      };

      // 3. Génération du fingerprint complet
      const fingerprint = this.fingerprintGenerator.generate(preprocessed.cleaned, methodsContext);

      // 4. Recherche parallèle: cache + expériences similaires
      const [cached, similarExperiences] = await Promise.all([
        this.config.agents.cache.enabled
          ? this.agents.cache.find(query)
          : { hit: false },
        this.experienceMemory.findSimilar(fingerprint, { limit: 5 })
      ]);

      // 5. Analyse des risques basée sur l'expérience
      const riskAnalysis = this.riskEngine.analyzeRisks(fingerprint, similarExperiences);

      // Afficher les warnings si présents
      if (riskAnalysis.warnings.length > 0 || riskAnalysis.blockedApproaches.length > 0) {
        section('Analyse des risques (basée sur l\'expérience)');
        console.log(this.riskEngine.formatRiskReport(riskAnalysis));
      }

      // 6. Créer un événement pour tracking
      currentEvent = this.experienceMemory.createEvent(query, fingerprint, methodsContext);

      // 7. Vérifier le cache (avec enrichissement expérience)
      if (cached.hit) {
        const enrichedCache = this.agents.cache.enrichWithExperience(cached, riskAnalysis);

        log('Orchestrator', `Question similaire trouvée en cache (${(cached.similarity * 100).toFixed(0)}%)`, 'success');

        if (enrichedCache.shouldRevalidate) {
          log('Orchestrator', '⚠️  Risque élevé détecté - revalidation recommandée', 'warning');
        } else {
      ...
      // 8. Analyse (avec contexte d'expérience injecté)
      const contextWithExperience = {
        ...methodsContext,
        experienceContext: {
          riskLevel: riskAnalysis.overallRiskLevel,
          warnings: riskAnalysis.warnings,
          blockedApproaches: riskAnalysis.blockedApproaches,
          recommendations: riskAnalysis.recommendations,
          similarExperiences: similarExperiences.map(e => ({
            similarity: e.similarity,
            outcome: e.event.outcome.status,
            lesson: e.event.cause.lesson
          }))
        }
      };

      const analysis = await this.agents.analysis.analyze(preprocessed, contextWithExperience);

      // 9. Propositions (sauf mode quick)
      let selectedProposal = null;
      let proposals = null;

      if (!this.options.quick) {
        proposals = await this.agents.proposal.generateProposals(analysis, contextWithExperience);
        ...
        if (!validation.approved) {
          ...
          this.experienceMemory.markFailure('cancelled', 'Annulé par l\'utilisateur', {
            category: 'prompt'
          });
        ...
        this.experienceMemory.updateDecision(
          validation.selected,
          selectedProposal?.title || 'quick_mode'
        );
      }

      // 10. Extraire le code des méthodes pertinentes (granularité maximale !)
      ...
      // 11. Générer le prompt optimisé (avec warnings injectés)
      ...
      // 15. Marquer comme succès et sauvegarder dans le cache
      if (result.success) {
        this.experienceMemory.markSuccess();
      ...
      // Enregistrer l'échec dans la mémoire d'expérience
      if (currentEvent) {
        this.experienceMemory.markFailure(
          error.name || 'error',
          error.message,
          { category: this.experienceMemory.categorizeError(error.name, error.message) }
        );
      }
