...
async function main() {
  ...
  console.log(`  ${COLORS.cyan}→${COLORS.reset} lib/agents.config.json`);
  ...
  const configPath = path.join(libDir, 'agents.config.json');
  
  // Backup si existe déjà
  if (fs.existsSync(configPath)) {
    const backupPath = configPath + '.backup';
    fs.copyFileSync(configPath, backupPath);
    log(`Backup créé: agents.config.json.backup`, 'warning');
  }
...
1. ${COLORS.cyan}Vérifiez la configuration${COLORS.reset}
   ${COLORS.yellow}→${COLORS.reset} Éditez lib/agents.config.json si nécessaire
