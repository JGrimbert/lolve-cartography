...
class CacheAgent {
  ...
  /**
   * Alias pour add() - compatibilité avec l'Orchestrator
   */
  store(query, response, metadata = {}) {
    return this.add(query, response, metadata);
  }
  ...
  /**
   * Enrichit un résultat de cache avec les données d'expérience
   * @param {Object} cacheResult - Résultat de find()
   * @param {Object} experienceData - Données de RiskInferenceEngine.analyzeRisks()
   * @returns {Object} Résultat enrichi avec warnings et risques
   */
  enrichWithExperience(cacheResult, experienceData) {
    if (!cacheResult.hit) {
      return {
        ...cacheResult,
    ...
    // Enrichir le hit cache avec les informations d'expérience
    return {
      ...cacheResult,
      experienceContext: experienceData,
      shouldRevalidate: experienceData.overallRiskLevel === 'high',
      warnings: experienceData.warnings || [],
