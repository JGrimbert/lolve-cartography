...
class AnalysisAgent {
  ...
  async analyze(request, context) {
    ...
    // Support both 'query' and 'cleaned' fields for compatibility
    const query = request.query || request.cleaned || request.original;
    const { intent, detectedTerms } = request;
    const relevantFiles = context.relevantFiles || context.methods || [];
