...
class MethodReinjector {
  ...
  parseModifiedFile(tempFilePath) {
    ...
    // Note: (?:\/\/ Score: [^\n]*\n)? ignore la ligne Score optionnelle (ne pas réinjecter)
    ...
    while ((match = methodRegex.exec(content)) !== null) {
      ...
      // Détecter le marqueur de suppression: // DELETE ou code vide
      const isDeleted = trimmedCode === '' ||
                        trimmedCode === '// DELETE' ||
                        trimmedCode.startsWith('// DELETE');
  ...
  detectChanges(originalSnapshot, modifiedMethods) {
    ...
    for (const [key, modData] of Object.entries(modifiedMethods)) {
      ...
      } else if (!modData.deleted) {
        // Nouvelle méthode (seulement si pas marquée deleted)
        changes.added.push({
          key,
          code: modData.code,
          fileInfo: modData.fileInfo
        });
      }
    }
  ...
  /**
   * Supprime une méthode d'un fichier
   */
  deleteMethodFromFile(filePath, oldCode) {
    let content = fs.readFileSync(filePath, 'utf-8');
    const normalized = this.snapshot.normalizeCode(oldCode);

    // Tentative 1: Suppression exacte
    if (content.includes(oldCode)) {
      // Supprimer la méthode et les lignes vides en trop
      content = content.replace(oldCode, '');
      content = content.replace(/\n{3,}/g, '\n\n'); // Max 2 lignes vides consécutives
      return { success: true, content, method: 'exact' };
    }

    // Tentative 2: Suppression avec normalisation
    const lines = content.split('\n');
    const oldLines = oldCode.split('\n').length;

    for (let i = 0; i < lines.length; i++) {
      const chunk = lines.slice(i, i + oldLines).join('\n');
      const chunkNorm = this.snapshot.normalizeCode(chunk);

      if (chunkNorm === normalized) {
        // Supprimer les lignes correspondantes
        lines.splice(i, oldLines);
        content = lines.join('\n');
        content = content.replace(/\n{3,}/g, '\n\n');
        return { success: true, content, method: 'normalized' };
      }
    ...
    return { success: false, content, method: 'failed' };
  }
  ...
  async reinject(snapshotPath, tempFilePath) {
    ...
    console.log(`   Supprimées: ${changes.deleted.length}`);
    ...
    // 5. Grouper par fichier (modifications + suppressions)
    const byFile = {};
    for (const change of changes.modified) {
      if (!byFile[change.file]) byFile[change.file] = { modifications: [], deletions: [] };
      byFile[change.file].modifications.push(change);
    }
    for (const change of changes.deleted) {
      if (!byFile[change.file]) byFile[change.file] = { modifications: [], deletions: [] };
      byFile[change.file].deletions.push(change);
    }
    ...
    for (const [file, fileChanges] of Object.entries(byFile)) {
      ...
      for (const change of fileChanges.modifications) {
        const result = this.replaceMethodInFile(filePath, change.oldCode, change.newCode);

        if (result.success) {
          content = result.content;
          fs.writeFileSync(filePath, content, 'utf-8'); // Écrire pour que les prochains remplacements utilisent le bon contenu
      ...
      for (const change of fileChanges.deletions) {
        const result = this.deleteMethodFromFile(filePath, change.oldCode);

        if (result.success) {
          content = result.content;
          fs.writeFileSync(filePath, content, 'utf-8');
          console.log(`   🗑️  ${change.key} supprimée (${result.method})`);
          results.success.push(change.key);
        } else {
          console.error(`   ❌ ${change.key} suppression échec`);
          results.failed.push({ key: change.key, score: 0 });
        }
      }
    ...
    // 8. Réindexer les fichiers modifiés
    if (results.success.length > 0 && !this.options.dryRun) {
      console.log(`\n🔄 Réindexation...`);
      try {
        const indexer = new MethodIndexer();
        indexer.loadIndex();

        const modifiedFiles = new Set(Object.keys(byFile));
        for (const file of modifiedFiles) {
          const filePath = originalSnapshot.files[file]?.path;
          if (filePath && fs.existsSync(filePath)) {
            indexer.indexFile(filePath);
            console.log(`   ✓ ${file} réindexé`);
          }
        }
        ...
        console.warn(`   ⚠️  Réindexation échouée: ${error.message}`);
      }
    }
