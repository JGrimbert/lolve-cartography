...
async function runAudit(targetFile = null) {
  log('AnnotationManager', '=== ANNOTATION AUDIT ===', 'info');
  ...
  for (const file of filtered) {
    ...
    for (const s of file.suggestions) {
      ...
      const missingTags = s.missing.map(m => `@${m}`).join(', ');

      console.log(`  ${icon} ${target}`);
      console.log(`     Missing: ${missingTags}`);
    }
  }

  console.log(`\n📊 Total: ${totalMissing} missing annotations in ${filtered.length} files`);
}

async function runSuggest(targetFile = null, options = {}) {
  ...
  for (const file of filtered) {
    ...
    for (const s of file.suggestions) {
      ...
      if (s.suggested.consumer) {
        console.log(`     + @consumer ${s.suggested.consumer.join(', ')}`);
      }
      if (s.suggested.context) {
        if (s.suggested.context.requires?.length > 0) {
          console.log(`     + @context requires: ${s.suggested.context.requires.join(', ')}`);
        }
        if (s.suggested.context.provides?.length > 0) {
          console.log(`     + @context provides: ${s.suggested.context.provides.join(', ')}`);
        }
      }
    }
  }

  console.log(`\n💾 Suggestions saved: ${CONFIG.suggestionsPath}`);
}
...
async function main() {
  ...
  switch (command) {
    case 'audit':
      await runAudit(options.file);
      break;

    case 'suggest':
      await runSuggest(options.file, options);
      break;
    ...
    default:
      console.log(`
Annotation Manager - JSDoc annotation management for LOLVE

Usage:
  node agents/annotation-manager.cjs <command> [options]
  npm run annotate:<command>

Commands:
  index              Generate/update method index (incremental)
  audit              List missing annotations
  suggest            Generate annotation suggestions
  apply              Generate a Markdown report with annotations to copy
  stats              Display annotation statistics
...
Examples:
  npm run annotate:audit
  npm run annotate:suggest -- --file Orb.js
  npm run annotate:apply -- --file Biblio.js
  npm run annotate:stats

  # Or directly:
  node agents/annotation-manager.cjs audit
  node agents/annotation-manager.cjs suggest --file Vertex.js
...
Other supported annotations:
  @effect creates: X, Y    Objects created by the method
  @effect mutates: X       Objects modified in place
  @effect emits: X         Events emitted
  @consumer X, Y           Classes that call this method
  @context requires: X     Contextual dependencies
  @context provides: X     What the method provides

Generated files:
  agents/.cache/annotation-suggestions.json   Suggestions in JSON
  agents/.cache/annotation-report.md          Report to copy
`);
  }
}
