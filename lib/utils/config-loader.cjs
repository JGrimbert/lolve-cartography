...
function loadProjectConfig(projectRoot = process.cwd()) {
  ...
  const configFilePath = path.join(projectRoot, '.lolve-cartography.json');
  if (fs.existsSync(configFilePath)) {
    try {
      userConfig = JSON.parse(fs.readFileSync(configFilePath, 'utf-8'));
      userConfig._source = '.lolve-cartography.json';
    } catch (e) {
      console.warn(`Warning: Invalid .lolve-cartography.json: ${e.message}`);
    }
  }

  // 2. Chercher dans package.json
  if (!userConfig._source) {
    const packageJsonPath = path.join(projectRoot, 'package.json');
    if (fs.existsSync(packageJsonPath)) {
      try {
        const pkg = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'));
        if (pkg['lolve-cartography']) {
          userConfig = pkg['lolve-cartography'];
          userConfig._source = 'package.json';
        }
        // Utiliser le nom du projet si non spécifié
        if (!userConfig.project?.name && pkg.name) {
          userConfig.project = userConfig.project || {};
          userConfig.project.name = pkg.name;
        }
      } catch (e) {
        // Ignorer
      }
  ...
  // 3. Chercher agents.config.json (legacy/local)
  if (!userConfig._source) {
    const legacyConfigPath = path.join(projectRoot, 'agents.config.json');
    if (fs.existsSync(legacyConfigPath)) {
      try {
        userConfig = JSON.parse(fs.readFileSync(legacyConfigPath, 'utf-8'));
        userConfig._source = 'agents.config.json';
      } catch (e) {
        // Ignorer
      }
    }
