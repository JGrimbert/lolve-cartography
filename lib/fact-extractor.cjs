    ...
    for (const sourceType of ['module', 'script']) {
      try {
        return parser.parse(content, {
          ecmaVersion: 'latest',
          sourceType,
          locations: isTS,   // acorn-typescript exige locations:true
          allowHashBang: true
        });
      } catch { /* essai suivant */ }
    }
...
class FactExtractor {
  ...
  _extractFromAST(facts) {
    ...
    // Index nom → [clé qualifiée] et fichier → [clés] pour résolution des appels
    const nameToKeys = new Map();  // 'searchMethods' → ['searchMethods', ...]
    const fileToKeys = new Map();  // 'src/lib/...' → ['atlas.GET', ...]
    for (const [key, method] of Object.entries(this.methodIndex.methods || {})) {
      const name = method.name;
      if (!nameToKeys.has(name)) nameToKeys.set(name, []);
      nameToKeys.get(name).push(key);
      if (method.file) {
        const rel = method.file.replace(/\\/g, '/');
        if (!fileToKeys.has(rel)) fileToKeys.set(rel, []);
        fileToKeys.get(rel).push(key);
      }
    }
    ...
    for (const filePath of files) {
      ...
      this._extractFromFunctionBodies(ast, nameToKeys, fileToKeys, relFile, facts);
    }
  }

  _extractImports(ast, relFile, facts) {
    walk(ast, (node) => {
      // ESM import (relatif ET alias projet : ./ $lib/ ~/...)
      if (node.type === 'ImportDeclaration' && node.source?.value) {
        const v = node.source.value;
        if (v.startsWith('.') || v.startsWith('$') || v.startsWith('~')) {
          facts.push({ entity: relFile, relation: 'IMPORTS', target: v });
        }
      }
      // CJS require
      if (
  ...
  // ─── Appels entre fonctions standalone ───────────────────────────────────────

  /**
   * Walk les corps des fonctions déclarées au top-level (pas de classes)
   * et détecte les appels à des fonctions connues du method-index.
   */
  _extractFromFunctionBodies(ast, nameToKeys, fileToKeys, relFile, facts) {
    const keysInFile = fileToKeys.get(relFile) || [];

    for (const topNode of ast.body) {
      let funcNode = null;
      let funcName = null;

      if (topNode.type === 'FunctionDeclaration' && topNode.id?.name) {
        funcNode = topNode; funcName = topNode.id.name;
      } else if (
        (topNode.type === 'ExportNamedDeclaration' || topNode.type === 'ExportDefaultDeclaration') &&
        topNode.declaration?.type === 'FunctionDeclaration' &&
        topNode.declaration.id?.name
      ) {
        funcNode = topNode.declaration; funcName = funcNode.id.name;
      }

      if (!funcNode || !funcName) continue;

      // Clé qualifiée de ce caller (ex: "atlas.GET" plutôt que "GET")
      const callerKey = keysInFile.find(k => {
        const m = this.methodIndex.methods[k];
        return m?.name === funcName;
      }) || funcName;
      ...
      walk(body, (node) => {
        ...
        if (node.type === 'CallExpression') {
          ...
          if (callee.type === 'Identifier' && callee.name !== funcName) {
            const targets = nameToKeys.get(callee.name);
            if (targets) {
              for (const target of targets) {
                if (target !== callerKey) {
                  facts.push({ entity: callerKey, relation: 'CALLS', target });
                }
              }
            }
          }
          // Appel chaîné : obj.method(...)  — on ne prend que si la méthode est connue
          if (callee.type === 'MemberExpression' && callee.property?.name) {
            const targets = nameToKeys.get(callee.property.name);
            if (targets) {
              for (const target of targets) {
                if (target !== callerKey) {
                  facts.push({ entity: callerKey, relation: 'CALLS', target });
